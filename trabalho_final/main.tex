...
A implementação típica combina um \texttt{chat model} com uma estratégia de memória (por exemplo, \texttt{ConversationBufferMemory}) para reter histórico relevante, um \textit{system prompt} que define o comportamento e integrações com bancos de dados ou APIs para recuperar informações específicas do usuário. Para produção, é comum instrumentar pipelines com logging, métricas e análises de conversação que alimentam ciclos de melhoria contínua (A/B tests, retreinos e ajustes de prompts). Essas práticas facilitam tanto o diagnóstico de falhas quanto a evolução do agente em produção. \cite{langchain2023docs, ji2023survey}
...
Sistemas de Question-Answering (QA) baseados em LangChain exploram amplamente o paradigma RAG (Retrieval-Augmented Generation) para responder perguntas sobre coleções documentais corporativas, como documentação técnica, manuais e relatórios. Ao recuperar passagens relevantes de uma base indexada e usar o LLM para gerar a resposta, esses sistemas entregam respostas fundamentadas em fontes verificáveis, reduzindo a incidência de \emph{hallucinations} e facilitando a citação de evidências. Aplicações típicas incluem assistentes para documentação interna, pesquisa legal e suporte a tomada de decisão. \cite{lewis2020retrieval, ji2023survey, anthopic2023claude}
...
A arquitetura padrão para QA inclui um \textit{document ingestion pipeline} (loaders e parsers), geração de embeddings e armazenamento em \textit{vector stores} para busca semântica, e um componente de \texttt{retriever} que seleciona trechos relevantes para o LLM. Chains prontos como \texttt{RetrievalQA} ou pipelines customizados orquestram a recuperação, a geração e um pós-processamento que formata respostas e acrescenta referências às fontes recuperadas. Para cenários em larga escala, práticas de engenharia (sharding de índices, aproximações para busca de vizinhança e monitoramento de latência) e trabalhos sobre embeddings e índices são relevantes para desempenho e custo. \cite{langchain2023docs, reimers2019sentence, johnson2019billion}
...
O LlamaIndex (anteriormente conhecido por nomes como GPT Index) concentra-se especificamente em indexação de dados e em facilitar pipelines de retrieval para aplicações RAG. Sua força está em oferecer interfaces relativamente simples para ingestão de documentos, construção de índices e recuperação semântica, tornando-o uma escolha natural quando o objetivo principal é construir soluções de QA ou search fundamentadas em documentos. Em comparação com LangChain, LlamaIndex tende a ter menos camadas de abstração genéricas e é mais direto para cenários onde retrieval é o ponto central. \cite{llamaindex2023, lewis2020retrieval}
...
O Semantic Kernel foca em orquestração de AI com ênfase em integração com o ecossistema Microsoft — incluindo integração nativa com Azure OpenAI. Conceitos centrais do kernel incluem \emph{Skills}, \emph{Planners} e \emph{Memory}, proporcionando um modelo arquitetural que favorece cenários empresariais e aplicações fortemente integradas à stack Microsoft. O suporte a múltiplas linguagens (C\#, Python, Java) e o enfoque em padrões enterprise tornam o Semantic Kernel atrativo para organizações já inseridas na plataforma Azure. \cite{semantickernel2023}
...
O Haystack é orientado para pipelines de NLP e sistemas de busca documentais em produção, com forte foco em QA, ingestão de documentos e integração com vector stores para busca semântica. É especialmente adequado para equipes que precisam de um produto maduro para search e question answering em produção, oferecendo componentes otimizados para indexação, recuperação e orquestração de pipelines de NLP. Em termos de foco, o Haystack tende a priorizar desempenho e estabilidade em search sobre capacidades conversacionais mais amplas. \cite{haystack2023, johnson2019billion}
