...
A estrutura do artigo segue a sequência: Seção~\ref{sec:fundamentos} (fundamentos e arquitetura), Seção~\ref{sec:componentes} (componentes principais), Seção~\ref{sec:aplicacoes} (aplicações práticas), Seção~\ref{sec:comparativo} (análise comparativa) e Seção~\ref{sec:conclusao} (conclusões e trabalhos futuros).
...
Em paralelo, formou-se um ecossistema complementar. O \textit{LangChain Core} permaneceu como a biblioteca principal; o \textit{LangSmith} surgiu como plataforma de depuração, avaliação e monitoramento de aplicações com LLMs; o \textit{LangServe} facilitou a publicação de \textit{chains} e agentes como serviços web; e o \textit{LangGraph} introduziu orquestração baseada em grafos para fluxos complexos e controlados por estado. Esses componentes reforçam a proposta \textit{end-to-end}, cobrindo do desenvolvimento local à observabilidade em produção \cite{langchain2023docs, langsmith2023}.
